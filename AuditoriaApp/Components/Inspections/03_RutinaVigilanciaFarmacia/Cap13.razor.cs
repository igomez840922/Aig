...
namespace AuditoriaApp.Components.Inspections._03_RutinaVigilanciaFarmacia
{
    public partial class Cap13
    {
        ...
        protected async Task FetchData()
        {
            try
            {
                if (Inspeccion.Inspeccion.InspRutinaVigFarmacia.InventarioMedicamento == null)
                {
                    Inspeccion.Inspeccion.InspRutinaVigFarmacia.InventarioMedicamento = new AUD_InventarioMedicamento();
                }
        ...
        protected async Task<bool> Save()
        {
            try
            {
                ...
                Inspeccion.Inspeccion.InspRutinaVigFarmacia.InventarioMedicamento.PendingUpdate = true;
        ...
        private async Task EditProduct(AUD_InvProducto data = null)
        {
            ...
            if (!result.Cancelled)
            {
                if (result.Data != null)
                {
                    ...
                    Inspeccion.Inspeccion.InspRutinaVigFarmacia.InventarioMedicamento.LProductos = Inspeccion.Inspeccion.InspRutinaVigFarmacia.InventarioMedicamento.LProductos?.Count > 0 ? Inspeccion.Inspeccion.InspRutinaVigFarmacia.InventarioMedicamento.LProductos : new List<AUD_InvProducto>();
                    if(!Inspeccion.Inspeccion.InspRutinaVigFarmacia.InventarioMedicamento.LProductos.Contains(actividad))
                        Inspeccion.Inspeccion.InspRutinaVigFarmacia.InventarioMedicamento.LProductos.Add(actividad);
                    await this.InvokeAsync(StateHasChanged);
                }
            }
        ...
        private async Task RemoveProduct(AUD_InvProducto data)
        {
            ...
            Inspeccion.Inspeccion.InspRutinaVigFarmacia.InventarioMedicamento.LProductos.Remove(data);
            await this.InvokeAsync(StateHasChanged);
        }
    }
}
