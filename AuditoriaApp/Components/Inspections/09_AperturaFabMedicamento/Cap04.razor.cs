...
namespace AuditoriaApp.Components.Inspections._09_AperturaFabMedicamento
{
    public partial class Cap04
    {
        ...
        protected async Task FetchData()
        {
            try
            {
                Inspeccion.Inspeccion.InspAperFabricante.ProdFabrican = Inspeccion.Inspeccion.InspAperFabricante.ProdFabrican != null ? Inspeccion.Inspeccion.InspAperFabricante.ProdFabrican : new AUD_InspAperFabricanteProdFabrican();
                await LoadData();
            }
            catch { }
            finally
        ...
        protected async Task<bool> Save()
        {
            try
            {
                ...
                Inspeccion.Inspeccion.InspAperFabricante.PendingUpdate = true;
                Inspeccion.Inspeccion.InspAperFabricante.ProdFabrican.PendingUpdate = true;
            ...
            try
            {
                ...
                if (!result.Cancelled)
                {
                    if (result.Data != null)
                    {
                        ...
                        Inspeccion.Inspeccion.InspAperFabricante.ProdFabrican.LAttachments = Inspeccion.Inspeccion.InspAperFabricante.ProdFabrican.LAttachments?.Count > 0 ? Inspeccion.Inspeccion.InspAperFabricante.ProdFabrican.LAttachments : new List<AttachmentTB>();
                        Inspeccion.Inspeccion.InspAperFabricante.ProdFabrican.LAttachments.Add(attachment);
                        await this.InvokeAsync(StateHasChanged);
                    }
                }
            }
            ...
            try
            {
             ...
             { nameof(Components.Dialog.DialogComponent.ContentText), string.Format("Está seguro desea eliminar el dato seleccionado?") }};
                ...
                if (!result.Cancelled)
                {
                    ...
                    if (succeeded)
                    {
                        Inspeccion.Inspeccion.InspAperFabricante.ProdFabrican.LAttachments.Remove(file);
                        await this.InvokeAsync(StateHasChanged);
                    }
                    else
                    {
