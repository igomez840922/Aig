...
namespace AuditoriaApp.Components.Inspections._1_AperturaUbicacionFarmacia
{
    public partial class Cap09
    {
        ...
        protected async Task FetchData()
        {
            try
            {                
                if (Inspeccion.Inspeccion.InspAperCambUbicFarm.DatosAreaAlmacenamiento == null)
                {
                    Inspeccion.Inspeccion.InspAperCambUbicFarm.Inicializa_DatosAreaAlmacenamiento();
                }                
                await LoadData();
            }
            catch { }
