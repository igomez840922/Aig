...
namespace AuditoriaApp.Components.Inspections._14_BpmFabNaturalesMed
{
    public partial class Cap04
    {
        ...
        protected async Task FetchData()
        {
            try
            {
                Inspeccion.Inspeccion.InspGuiBPMFabNatMedicina.OtrosFuncionarios = Inspeccion.Inspeccion.InspGuiBPMFabNatMedicina.OtrosFuncionarios != null ? Inspeccion.Inspeccion.InspGuiBPMFabNatMedicina.OtrosFuncionarios : new AUD_OtrosFuncionarios();
                await LoadData();
            }
            catch { }
            finally
        ...
        protected async Task<bool> Save()
        {
            try
            {
                ...
                Inspeccion.Inspeccion.InspGuiBPMFabNatMedicina.OtrosFuncionarios.PendingUpdate = true;
        ...
        private async Task EditParticipant(DatosPersona data = null)
        {
            ...
            if (!result.Cancelled)
            {
                if (result.Data != null)
                {
                    ...
                    Inspeccion.Inspeccion.InspGuiBPMFabNatMedicina.OtrosFuncionarios.LPersona = Inspeccion.Inspeccion.InspGuiBPMFabNatMedicina.OtrosFuncionarios.LPersona?.Count > 0 ? Inspeccion.Inspeccion.InspGuiBPMFabNatMedicina.OtrosFuncionarios.LPersona : new List<DatosPersona>();
                    if (!Inspeccion.Inspeccion.InspGuiBPMFabNatMedicina.OtrosFuncionarios.LPersona.Contains(actividad))
                        Inspeccion.Inspeccion.InspGuiBPMFabNatMedicina.OtrosFuncionarios.LPersona.Add(actividad);
                    await this.InvokeAsync(StateHasChanged);
                }
            }
        ...
        private async Task RemoveParticipant(DatosPersona data)
        {
            ...
            Inspeccion.Inspeccion.InspGuiBPMFabNatMedicina.OtrosFuncionarios.LPersona.Remove(data);
            await this.InvokeAsync(StateHasChanged);
        }
