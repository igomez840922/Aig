        ...
        public async Task<Stream> ExportToExcel(GenericModel<FMV_EsaviTB> model)
        {
            try
            {
                ...
                if (model.Ldata != null && model.Ldata.Count > 0)
                {
                    ...
                    ws.Cell(1, 5).Value = "Código Externo";
                    ws.Cell(1, 6).Value = "Código del CNFV";
                    ws.Cell(1, 7).Value = "Fecha de Recibido (CNFV)";
                    ws.Cell(1, 8).Value = "Fecha Entrega a Evaluador";
                    ws.Cell(1, 9).Value = "Evaluador";
                    ...
                    foreach (var data in model.Ldata)
                    {
                        foreach (var prod in data.LNotificaciones)
                        {
                            ...
                            ws.Cell(row + 1, 12).Value = data.OtrosDiagnosticos;
                            ws.Cell(row + 1, 13).Value = DataModel.Helper.Helper.GetDescription(data.Sexo);
                            ws.Cell(row + 1, 14).Value = data.Edad;
                            ws.Cell(row + 1, 15).Value = data.HistoriaClinica;
                            ws.Cell(row + 1, 16).Value = data.DatosLab;
                            ws.Cell(row + 1, 17).Value = data.NombreCompletoPersona;
                            ws.Cell(row + 1, 18).Value = data.InicialesPersona;
                            ws.Cell(row + 1, 19).Value = data.Cedula;
                            ws.Cell(row + 1, 20).Value = data.MedicamentoContaminante;
                            ws.Cell(row + 1, 21).Value = DataModel.Helper.Helper.GetDescription(data.DetallesCaso);
                            ws.Cell(row + 1, 22).Value = data.FechaEvalua?.ToString("dd/MM/yyyy") ?? "";
                            ws.Cell(row + 1, 23).Value = DataModel.Helper.Helper.GetDescription(data.Estatus);
                            ws.Cell(row + 1, 24).Value = data.Observaciones;
                            ws.Cell(row + 1, 25).Value = DataModel.Helper.Helper.GetDescription(prod.HayEsavi);
                            ws.Cell(row + 1, 26).Value = prod.FechaEsavi?.ToString("dd/MM/yyyy") ?? "";
                            ws.Cell(row + 1, 27).Value = DataModel.Helper.Helper.GetDescription(prod.Desenlace);
                            ws.Cell(row + 1, 28).Value = prod.EsaviDescripcion;
                            ws.Cell(row + 1, 29).Value = prod.TerminoWhoArt;
                            ws.Cell(row + 1, 30).Value = DataModel.Helper.Helper.GetDescription(prod.SOC);
                            ws.Cell(row + 1, 31).Value = prod.Intensidad;
                            ws.Cell(row + 1, 32).Value = prod.Gravedad;
                            ws.Cell(row + 1, 33).Value = DataModel.Helper.Helper.GetDescription(prod.OtrosCriterios);
                            ws.Cell(row + 1, 34).Value = prod.ElegibilidadGravedad;
                            ws.Cell(row + 1, 35).Value = prod.ElegibilidadOtroCriterio;
                            ws.Cell(row + 1, 36).Value = prod.ElegibleEvaluacionCausal;
                            ws.Cell(row + 1, 37).Value = DataModel.Helper.Helper.GetDescription(prod.ProbabilidadAsociacion);
                            ws.Cell(row + 1, 38).Value = prod.VacunaComercial;
                            ws.Cell(row + 1, 39).Value = prod.DescripVacuna;
                            ws.Cell(row + 1, 40).Value = prod.Lote;
                            ws.Cell(row + 1, 41).Value = prod.FechaExp?.ToString("dd/MM/yyyy") ?? "";
                            ws.Cell(row + 1, 42).Value = prod.RegSanitario;
                            ws.Cell(row + 1, 43).Value = prod.FechaVacunacion?.ToString("dd/MM/yyyy") ?? "";
                            ws.Cell(row + 1, 44).Value = prod.Indicaciones;
                            ws.Cell(row + 1, 45).Value = prod.DosisViaAdmin;
                            ws.Cell(row + 1, 46).Value = prod.DosisEsavi;
                        }
