...
namespace Aig.FarmacoVigilancia.Pages.ESAVI2
{
    public partial class Index
    {
        ...
        GenericModel<FMV_Esavi2TB> dataModel { get; set; } = new GenericModel<FMV_Esavi2TB>()
        { Data = new FMV_Esavi2TB() };
        ...
        private async Task OnEdit(long id)
        {
            ...
            if (result == null)
            {
                result = new FMV_Esavi2TB();
            }
            dataModel.Data = result;
            //Aig.FarmacoVigilancia.Events.Ram.AddEdit_CloseEvent
            bus.Subscribe<Aig.FarmacoVigilancia.Events.ESAVI2.AddEditEvent>(AddEdit_CloseHandler);

            await this.InvokeAsync(StateHasChanged);
        }
        private void AddEdit_CloseHandler(MessageArgs args)
        {
            bus.UnSubscribe<Aig.FarmacoVigilancia.Events.ESAVI2.AddEditEvent>(AddEdit_CloseHandler);

            OpenAddEditDialog = false;
            var message = args.GetMessage<Aig.FarmacoVigilancia.Events.ESAVI2.AddEditEvent>();

            FetchData();
        }

        private async Task OnDelete(FMV_Esavi2TB data)
        {
