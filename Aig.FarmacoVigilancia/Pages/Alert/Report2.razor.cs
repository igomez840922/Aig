...
namespace Aig.FarmacoVigilancia.Pages.Alert
{
    public partial class Report2
    {
        ...
        [Inject]
        IAlertaService ftService { get; set; }
        [Inject]
        ...
        protected async Task FetchData()
        {
            ...
            var data = await ftService.Report2(dataModel);
            if (data != null)
            {
                dataModel = data;
            }
        ...
        protected async Task ExportToExcel() {
            Stream stream = await ftService.ExportToExcelRpt(dataModel, 2);
            if (stream != null) {
                await blazorDownloadFileService.DownloadFile("Reporte.xlsx", stream, "application/actet-stream");
            }
        }
