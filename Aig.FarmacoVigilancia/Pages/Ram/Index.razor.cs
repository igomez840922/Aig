...
namespace Aig.FarmacoVigilancia.Pages.Ram
{   
    public partial class Index
    {
        ...
        private async Task OnEdit(long id)
        {
            ...
            if (result == null)
            {
                result = new FMV_RamTB();
            }
            dataModel.Data = result;
            //Aig.FarmacoVigilancia.Events.Ram.AddEdit_CloseEvent
            bus.Subscribe<Aig.FarmacoVigilancia.Events.Ram.AddEdit_CloseEvent>(AddEdit_CloseHandler);

            await this.InvokeAsync(StateHasChanged);
        }
        private void AddEdit_CloseHandler(MessageArgs args)
        {
            bus.UnSubscribe<Aig.FarmacoVigilancia.Events.Ram.AddEdit_CloseEvent>(AddEdit_CloseHandler);

            OpenAddEditDialog = false;
            var message = args.GetMessage<Aig.FarmacoVigilancia.Events.Ram.AddEdit_CloseEvent>();

            FetchData();
        }

        private async Task OnDelete(FMV_RamTB data)
        {
