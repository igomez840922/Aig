...
namespace Aig.FarmacoVigilancia.Pages.Settings.NotaDestino
{
    public partial class Index
    {
        ...
        [Inject]
        INotaDestinoService mainService { get; set; }
        [Inject]
        IBlazorDownloadFileService blazorDownloadFileService { get; set; }
        GenericModel<DataModel.FMV_NotaDestinoTB> dataModel { get; set; } = new GenericModel<DataModel.FMV_NotaDestinoTB>()
        { Data = new DataModel.FMV_NotaDestinoTB() };
        ...
        private async Task OnEdit(long id)
        {
            ...
            if (result == null)
            {
                result = new DataModel.FMV_NotaDestinoTB();
            }
            dataModel.Data = result;
            //Aig.FarmacoVigilancia.Events.Ram.AddEdit_CloseEvent
            bus.Subscribe<Aig.FarmacoVigilancia.Events.NotaDestino.CloseEvent>(AddEdit_CloseHandler);

            await this.InvokeAsync(StateHasChanged);
        }
        private void AddEdit_CloseHandler(MessageArgs args)
        {
            bus.UnSubscribe<Aig.FarmacoVigilancia.Events.NotaDestino.CloseEvent>(AddEdit_CloseHandler);

            OpenAddEditDialog = false;
            var message = args.GetMessage<Aig.FarmacoVigilancia.Events.NotaDestino.CloseEvent>();

            FetchData();
        }

        private async Task OnDelete(DataModel.FMV_NotaDestinoTB data)
        {
