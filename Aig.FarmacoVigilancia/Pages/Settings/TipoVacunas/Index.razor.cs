...
namespace Aig.FarmacoVigilancia.Pages.Settings.TipoVacunas
{
    public partial class Index
    {
        ...
        GenericModel<TipoVacunaTB> dataModel { get; set; } = new GenericModel<TipoVacunaTB>()
        { Data = new TipoVacunaTB() };
        ...
        private async Task OnEdit(long id)
        {
            ...
            if (result == null)
            {
                result = new TipoVacunaTB();
            }
            dataModel.Data = result;
            //Aig.FarmacoVigilancia.Events.Ram.AddEdit_CloseEvent
            bus.Subscribe<Aig.FarmacoVigilancia.Events.TipoVacuna.AddEditEvent>(AddEdit_CloseHandler);

            await this.InvokeAsync(StateHasChanged);
        }
        private void AddEdit_CloseHandler(MessageArgs args)
        {
            bus.UnSubscribe<Aig.FarmacoVigilancia.Events.TipoVacuna.AddEditEvent>(AddEdit_CloseHandler);

            OpenAddEditDialog = false;
            var message = args.GetMessage<Aig.FarmacoVigilancia.Events.TipoVacuna.AddEditEvent>();

            FetchData();
        }

        private async Task OnDelete(TipoVacunaTB data)
        {
