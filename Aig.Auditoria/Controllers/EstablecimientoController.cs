    ...
    public class EstablecimientoController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetByNumber(string number)
        {
            ...
            try {
                if (!string.IsNullOrEmpty(number))
                {
                    ...
                    try {
                        long Id = long.Parse(number);
                        data = dalService.Get<AUD_EstablecimientoTB>(Id);
                    }
                    catch { }
                    if (data != null)
                    {
                    ...
                    {
                        data = dalService.Find<AUD_EstablecimientoTB>(x => x.NumLicencia == number);
                        if (data != null)
                        {
                            return Ok(data);
                        }
        ...
        public async Task<IActionResult> SaveUpdate([FromBody] AUD_EstablecimientoTB establecimiento)
        {
            ...
            if (establecimiento?.Provincia != null)
            {
                ...
                {
                    establecimiento.ProvinciaId = dalService.Find<ProvinciaTB>(x => x.Nombre.Contains(establecimiento.Provincia.Nombre))?.Id ?? 0;
                }
            ...
            if (establecimiento?.Distrito != null)
            {
                ...
                {
                    establecimiento.DistritoId = dalService.Find<DistritoTB>(x => x.Nombre.Contains(establecimiento.Distrito.Nombre))?.Id ?? 0;
                }
            ...
            if (establecimiento?.Corregimiento != null)
            {
                ...
                {
                    establecimiento.CorregimientoId = dalService.Find<CorregimientoTB>(x => x.Nombre.Contains(establecimiento.Corregimiento.Nombre))?.Id ?? 0;
                }
