        ...
        public async Task<Stream> ExportToExcel(GenericModel<AUD_ProdRetiroRetencionTB> model)
        {
            try
            {
                ...
                if(model.Ldata!=null && model.Ldata.Count > 0)
                {
                    ...
                    ws.Cell(1, 2).Value = "Fecha de Inspección";
                    ws.Cell(1, 3).Value = "Establecimientos";
                    ws.Cell(1, 4).Value = "Número de Licencia";
                    ws.Cell(1, 5).Value = "Número de Acta";
                    ws.Cell(1, 6).Value = "Nombre";
                    ws.Cell(1, 7).Value = "No. Registro Sanitario";
                    ws.Cell(1, 8).Value = "Presentación Comercial";
                    ws.Cell(1, 9).Value = "Fabricante";
                    ws.Cell(1, 10).Value = "País";
                    ws.Cell(1, 11).Value = "Lote";
                    ws.Cell(1, 12).Value = "Fecha Exp.";
                    ws.Cell(1, 13).Value = "Cantidad Retenida";
                    ws.Cell(1, 14).Value = "Cantidad Retirada";
                    ws.Cell(1, 15).Value = "Destino del Producto";
                    ...
                    for (int row = 1; row <= model.Ldata.Count; row++)
                    {
                        ...
                        ws.Cell(row + 1, 2).Value = prod.FrmRetiroRetencion?.Inspeccion?.FechaInicio.ToString("dd/MM/yyyy") ?? "";
                        ws.Cell(row + 1, 3).Value = prod.FrmRetiroRetencion?.Inspeccion?.Establecimiento?.Nombre ?? "";
                        ws.Cell(row + 1, 4).Value = prod.FrmRetiroRetencion?.Inspeccion?.LicenseNumber ?? "";
                        ws.Cell(row + 1, 5).Value = prod.FrmRetiroRetencion?.Inspeccion?.NumActa ?? "";
                        ws.Cell(row + 1, 6).Value = prod.Nombre;
                        ws.Cell(row + 1, 7).Value = prod.RegSanitario;
                        ws.Cell(row + 1, 8).Value = prod.PresentacionComercial;
                        ws.Cell(row + 1, 9).Value = prod.Fabricante;
                        ws.Cell(row + 1, 10).Value = prod.Pais;
                        ws.Cell(row + 1, 11).Value = prod.Lote;
                        ws.Cell(row + 1, 12).Value = prod.FechaExp?.ToString("dd/MM/yyyy") ?? "";
                        ws.Cell(row + 1, 13).Value = prod.CantidadRetenida;
                        ws.Cell(row + 1, 14).Value = prod.CantidadRetirada;
                        ws.Cell(row + 1, 15).Value = prod.Destino;
                    }
