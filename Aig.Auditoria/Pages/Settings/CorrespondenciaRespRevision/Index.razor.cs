...
namespace Aig.Auditoria.Pages.Settings.CorrespondenciaRespRevision
{
    ...
    public partial class Index
    {
        ...
        [Inject]
        ICorrespondenciaRespRevisionService mainService { get; set; }        
        
        GenericModel<AUD_CorrespondenciaRespRevisionTB> dataModel { get; set; } = new GenericModel<AUD_CorrespondenciaRespRevisionTB>()
        { Data = new AUD_CorrespondenciaRespRevisionTB() };
        ...
        private async Task OnEdit(long id)
        {
            ...
            if (result == null)
            {
                result = new AUD_CorrespondenciaRespRevisionTB();
            }
            dataModel.Data = result;

            bus.Subscribe<Aig.Auditoria.Events.CorrespondenciaRespRevision.AddEditEvent>(AddEditEventHandler);

            await this.InvokeAsync(StateHasChanged);
        }
        private void AddEditEventHandler(MessageArgs args)
        {
            bus.UnSubscribe<Aig.Auditoria.Events.CorrespondenciaRespRevision.AddEditEvent>(AddEditEventHandler);

            OpenAddEditDialog = false;
            var message = args.GetMessage<Aig.Auditoria.Events.CorrespondenciaRespRevision.AddEditEvent>();

            FetchData();
        }

        private async Task OnDelete(AUD_CorrespondenciaRespRevisionTB data)
        {
