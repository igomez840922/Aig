...
namespace Aig.Auditoria.Pages.Settings.CorrespondenciaContacto
{
    ...
    public partial class Index
    {
        ...
        [Inject]
        ICorrespondenciaContactoService mainService { get; set; }        
        
        GenericModel<AUD_CorrespondenciaContactoTB> dataModel { get; set; } = new GenericModel<AUD_CorrespondenciaContactoTB>()
        { Data = new AUD_CorrespondenciaContactoTB() };
        ...
        private async Task OnEdit(long id)
        {
            ...
            if (result == null)
            {
                result = new AUD_CorrespondenciaContactoTB();
            }
            dataModel.Data = result;

            bus.Subscribe<Aig.Auditoria.Events.CorrespondenciaContacto.AddEditEvent>(AddEditEventHandler);

            await this.InvokeAsync(StateHasChanged);
        }
        private void AddEditEventHandler(MessageArgs args)
        {
            bus.UnSubscribe<Aig.Auditoria.Events.CorrespondenciaContacto.AddEditEvent>(AddEditEventHandler);

            OpenAddEditDialog = false;
            var message = args.GetMessage<Aig.Auditoria.Events.CorrespondenciaContacto.AddEditEvent>();

            FetchData();
        }

        private async Task OnDelete(AUD_CorrespondenciaContactoTB data)
        {
