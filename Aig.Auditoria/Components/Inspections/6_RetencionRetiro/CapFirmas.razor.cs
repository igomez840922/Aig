...
namespace Aig.Auditoria.Components.Inspections._6_RetencionRetiro
{
    public partial class CapFirmas
    {
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_RetiroRetencion_Frima(Inspeccion);
                if (result != null)
                {
        ...
        async Task DelayToShowSignasure()
        {
            ...
            if(SignaturePad5!=null)
                SignaturePad5.Image = Inspeccion.InspRetiroRetencion?.DatosAtendidosPor?.Firma??null;
            //if (SignaturePad6 != null)
            //    SignaturePad6.Image = Inspeccion.InspRetiroRetencion?.DatosRegente?.Firma ?? null;
        ...
        protected async Task OnSignatureChange5(ChangeEventArgs eventArgs)
        {
            ...
            Inspeccion.InspRetiroRetencion.DatosAtendidosPor.Firma = await SignaturePad5.ToDataURL(signatureType);
        }
        ...
        //        var signatureType = (Mobsites.Blazor.SignaturePad.SupportedSaveAsTypes)Enum.Parse(typeof(Mobsites.Blazor.SignaturePad.SupportedSaveAsTypes), eventArgs.Value as string);
        //    }
        //    Inspeccion.InspRetiroRetencion.DatosAtendidosPor.Firma = await SignaturePad6.ToDataURL(signatureType);
        //}
