...
{   
    public partial class GuiaBPMProductosNatMedicinales
    {
        [Inject]
        ...
        protected async Task FetchData()
        {
            ...
            if (Inspeccion != null)
            {
                ...
                if (signaturePad6 != null)
                    signaturePad6.Image = Inspeccion.InspGuiBPMFabNatMedicina.RegenteFarmaceutico.Firma;

                foreach (var partic in Inspeccion.InspGuiBPMFabNatMedicina.DatosConclusiones.LParticipantes)
                {
                    try
                    {
                        lSignaturePads[Inspeccion.InspGuiBPMFabNatMedicina.DatosConclusiones.LParticipantes.IndexOf(partic)].Image = partic.Firma;
                    }
                    catch (Exception ex) { }
                }
            }
        ...
        {
            if (!showSignasure && Inspeccion.InspGuiBPMFabNatMedicina.DatosConclusiones.LParticipantes.Count > 0)
            {
        ...
        protected async Task OnSignatureChange5(ChangeEventArgs eventArgs)
        {
            ...
            Inspeccion.InspGuiBPMFabNatMedicina.RepresentLegal.Firma = await signaturePad5.ToDataURL(signatureType);
        }
        protected async Task RemoveSignatureImg5()
        {
            Inspeccion.InspGuiBPMFabNatMedicina.RepresentLegal.Firma = null;
        ...
        protected async Task OnSignatureChange6(ChangeEventArgs eventArgs)
        {
            ...
            Inspeccion.InspGuiBPMFabNatMedicina.RegenteFarmaceutico.Firma = await signaturePad6.ToDataURL(signatureType);
        }
        protected async Task RemoveSignatureImg6()
        {
            Inspeccion.InspGuiBPMFabNatMedicina.RegenteFarmaceutico.Firma = null;
        ...
        protected async Task OnSignatureChange(Participante _participante)
        {
            ...
            var _signaturePad = lSignaturePads[Inspeccion.InspGuiBPMFabNatMedicina.DatosConclusiones.LParticipantes.IndexOf(_participante)];
        ...
        protected async Task RemoveParticipant(Participante _participante)
        {
            if (_participante != null)
            {
                try
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.DatosConclusiones.LParticipantes.Remove(_participante);
                }
                catch { }
        ...
        private void ParticipantsAddEdit_CloseEventHandler(MessageArgs args)
        {
            ...
            {
                if (!Inspeccion.InspGuiBPMFabNatMedicina.DatosConclusiones.LParticipantes.Contains(message.Data))
                    Inspeccion.InspGuiBPMFabNatMedicina.DatosConclusiones.LParticipantes.Add(message.Data);
            }
        ...
        protected async Task RemovePersona(DataModel.DatosPersona _datosPersona)
        {
            if (_datosPersona != null)
            {
                try
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.OtrosFuncionarios.LPersona.Remove(_datosPersona);
                }
                catch { }
