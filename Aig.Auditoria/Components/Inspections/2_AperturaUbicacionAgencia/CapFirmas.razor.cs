...
namespace Aig.Auditoria.Components.Inspections._2_AperturaUbicacionAgencia
{
    public partial class CapFirmas
    {
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_AperCamUbicAgencia_Frima(Inspeccion);
                if (result != null)
                {
        ...
        async Task DelayToShowSignasure()
        {
            ...
            if(SignaturePad5!=null)
                SignaturePad5.Image = Inspeccion.InspAperCambUbicAgen?.DatosSolicitante?.Firma??null;
            if (SignaturePad6 != null)
                SignaturePad6.Image = Inspeccion.InspAperCambUbicAgen?.DatosRegente?.Firma ?? null;
        ...
        protected async Task OnSignatureChange5(ChangeEventArgs eventArgs)
        {
            ...
            Inspeccion.InspAperCambUbicAgen.DatosSolicitante.Firma = await SignaturePad5.ToDataURL(signatureType);
        }
        protected async Task RemoveSignatureImg5()
        {
            Inspeccion.InspAperCambUbicAgen.DatosSolicitante.Firma = null;
