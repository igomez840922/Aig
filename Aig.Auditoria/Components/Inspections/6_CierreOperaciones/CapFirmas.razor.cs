...
namespace Aig.Auditoria.Components.Inspections._6_CierreOperaciones
{
    public partial class CapFirmas
    {
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_CierreOperacion_Frima(Inspeccion);
                if (result != null)
                {
        ...
        async Task DelayToShowSignasure()
        {
            ...
            if(signaturePad5!=null)
                signaturePad5.Image = Inspeccion.InspCierreOperacion?.DatosRepresentLegal?.Firma??null;
            //if (signaturePad6 != null)
            //    signaturePad6.Image = Inspeccion.InspCierreOperacion?.DatosRegente?.Firma ?? null;
        ...
        protected async Task OnSignatureChange5(ChangeEventArgs eventArgs)
        {
            ...
            Inspeccion.InspCierreOperacion.DatosRepresentLegal.Firma = await signaturePad5.ToDataURL(signatureType);
        }
        protected async Task RemoveSignatureImg5()
        {
            Inspeccion.InspCierreOperacion.DatosRepresentLegal.Firma = null;
        ...
        //        var signatureType = (SignaturePad.SupportedSaveAsTypes)Enum.Parse(typeof(SignaturePad.SupportedSaveAsTypes), eventArgs.Value as string);
        //    }
        //    Inspeccion.InspCierreOperacion.DatosAtendidosPor.Firma = await signaturePad6.ToDataURL(signatureType);
        //}
