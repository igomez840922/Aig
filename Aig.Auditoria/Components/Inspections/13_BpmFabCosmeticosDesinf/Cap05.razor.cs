    ...
    public partial class Cap05
    {
        ...
        {            
            ...
            if (Inspeccion != null)
            {
                ...
                if (Inspeccion.InspGuiBPMFabCosmeticoMed.RequisitosLegales == null)
                {
                    Inspeccion.InspGuiBPMFabCosmeticoMed.Inicializa_RequisitosLegales();
                }
                if (Inspeccion.InspGuiBPMFabCosmeticoMed.ClasifActComerciales == null)
                {
                    Inspeccion.InspGuiBPMFabCosmeticoMed.Inicializa_ClasifActComerciales();
                }
                if (Inspeccion.InspGuiBPMFabCosmeticoMed.ClasifEstablecimiento == null)
                {
                    Inspeccion.InspGuiBPMFabCosmeticoMed.Inicializa_ClasifEstablecimiento();
                }
            }
            else { Cancel(); }
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_BpmFabCosmeticosDesinf_Cap5(Inspeccion);
                if (result != null)
                {
