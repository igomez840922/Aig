    ...
    public partial class Cap09
    {
        ...
        {            
            ...
            if (Inspeccion != null)
            {
                ...
                if (Inspeccion.InspGuiBPMFabNatMedicina.Generalidades8 == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_Generalidades8();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.Calibracion == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_Calibracion();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.SistemaAgua == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_SistemaAgua();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.SistemaAire == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_SistemaAire();
                }
            }
            else { Cancel(); }
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_BpmFabNaturalesMed_Cap9(Inspeccion);
                if (result != null)
                {
