    ...
    public partial class Cap16
    {
        ...
        {            
            ...
            if (Inspeccion != null)
            {
                ...
                if (Inspeccion.InspGuiBPMFabNatMedicina.Generalidades15 == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_Generalidades15();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.Contratante == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_Contratante();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.Contratista == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_Contratista();
                }
            }
            else { Cancel(); }
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_BpmFabNaturalesMed_Cap16(Inspeccion);
                if (result != null)
                {
