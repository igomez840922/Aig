    ...
    public partial class Cap14
    {
        ...
        {            
            ...
            if (Inspeccion != null)
            {
                ...
                if (Inspeccion.InspGuiBPMFabNatMedicina.Generalidades13 == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_Generalidades13();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.Muestreo == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_Muestreo();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.MetodologiaAnalitica == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_MetodologiaAnalitica();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.MaterialesReferencia == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_MaterialesReferencia();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.Estabilidad == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_Estabilidad();
                }
            }
            else { Cancel(); }
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_BpmFabNaturalesMed_Cap14(Inspeccion);
                if (result != null)
                {
