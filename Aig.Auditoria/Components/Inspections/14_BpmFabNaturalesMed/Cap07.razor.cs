    ...
    public partial class Cap07
    {
        ...
        {            
            ...
            if (Inspeccion != null)
            {
                ...
                if (Inspeccion.InspGuiBPMFabNatMedicina.Organizacion == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_Organizacion();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.Personal == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_Personal();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.ResponPersonal == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_ResponPersonal();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.Capacitacion == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_Capacitacion();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.HigieneSalud == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_HigieneSalud();
                }
            }
            else { Cancel(); }
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_BpmFabNaturalesMed_Cap7(Inspeccion);
                if (result != null)
                {
