    ...
    public partial class Cap11
    {
        ...
        {            
            ...
            if (Inspeccion != null)
            {
                ...
                if (Inspeccion.InspGuiBPMFabNatMedicina.Generalidades10 == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_Generalidades10();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.DocumentosExigido == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_DocumentosExigido();
                }
                if (Inspeccion.InspGuiBPMFabNatMedicina.ProcedimientoReg == null)
                {
                    Inspeccion.InspGuiBPMFabNatMedicina.Inicializa_ProcedimientoReg();
                }
            }
            else { Cancel(); }
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_BpmFabNaturalesMed_Cap11(Inspeccion);
                if (result != null)
                {
