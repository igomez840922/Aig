...
namespace Aig.Auditoria.Components.Inspections._3_RutinaVigilanciaFarmacia
{
    public partial class Cap09
    {
        ...
        protected async Task FetchData()
        {
            try
            {
                ...
                if (Inspeccion != null)
                {
                    ...
                    if (Inspeccion.InspRutinaVigFarmacia.RegMovimientoExistencia == null)
                    {
                        Inspeccion.InspRutinaVigFarmacia.Inicializa_RegMovimientoExistencia();
                    }
                }
                else { Cancel(); }
            }
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_RutinaVigilanciaFarmacia_Cap9(Inspeccion);
                if (result != null)
                {
