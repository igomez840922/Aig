...
namespace Aig.Auditoria.Components.Inspections._3_RutinaVigilanciaFarmacia
{
    public partial class CapFirmas
    {
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_RutinaVigilanciaFarmacia_Frima(Inspeccion);
                if (result != null)
                {
        ...
        protected async Task OnSignature(string signature)
        {
            try
            {
                switch (signatureTo)
                {
                    case 1:
                        {
                            Inspeccion.InspRutinaVigFarmacia.DatosRepresentLegal.Firma = signature;
                            break;
                        }
                    case 2:
                        {
                            Inspeccion.InspRutinaVigFarmacia.DatosRegente.Firma = signature;
                            break;
                        }
                    case 3:
                        {
