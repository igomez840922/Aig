...
namespace Aig.Auditoria.Components.Inspections._3_RutinaVigilanciaFarmacia
{
    public partial class Cap10
    {
        ...
        protected async Task FetchData()
        {
            ...
            if (Inspeccion != null)
            {
                switch (Inspeccion.StatusInspecciones)
                {
                    ...
                    if (Inspeccion.InspRutinaVigFarmacia.AreaAlmacenMedicamentos == null)
                    {
                        Inspeccion.InspRutinaVigFarmacia.Inicializa_AreaAlmacenMedicamentos();
                    }
                }
                else { Cancel(); }
            }
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_RutinaVigilanciaFarmacia_Cap10(Inspeccion);
                if (result != null)
                {
