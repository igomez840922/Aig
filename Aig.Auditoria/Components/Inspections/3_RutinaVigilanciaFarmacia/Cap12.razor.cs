...
namespace Aig.Auditoria.Components.Inspections._3_RutinaVigilanciaFarmacia
{
    public partial class Cap12
    {
        ...
        protected async Task FetchData()
        {
            ...
            if (Inspeccion != null)
            {
                switch (Inspeccion.StatusInspecciones)
                {
                    ...
                    {
                        Inspeccion.InspRutinaVigFarmacia.InventarioMedicamento = new AUD_InventarioMedicamento();
                    }
                }
                else { Cancel(); }
            }
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_RutinaVigilanciaFarmacia_Cap12(Inspeccion);
                if (result != null)
                {
        ...
        protected async Task RemoveProduct(AUD_InvProducto product)
        {
            if (product != null)
            {
                Inspeccion.InspRutinaVigFarmacia.InventarioMedicamento.LProductos.Remove(product);
            }
            await this.InvokeAsync(StateHasChanged);
        }
        ...
        private void AddEditCloseEventHandler(MessageArgs args)
        {
            ...
            {                
                if (!Inspeccion.InspRutinaVigFarmacia.InventarioMedicamento.LProductos.Contains(message.Data))
                    Inspeccion.InspRutinaVigFarmacia.InventarioMedicamento.LProductos.Add(message.Data);
