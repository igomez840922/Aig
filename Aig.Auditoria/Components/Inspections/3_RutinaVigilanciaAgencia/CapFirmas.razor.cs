...
namespace Aig.Auditoria.Components.Inspections._3_RutinaVigilanciaAgencia
{
    public partial class CapFirmas
    {
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_RutinaVigilanciaAgencia_Frima(Inspeccion);
                if (result != null)
                {
        ...
        async Task DelayToShowSignasure()
        {
            ...
            if(signaturePad5!=null)
                signaturePad5.Image = Inspeccion.InspRutinaVigAgencia?.DatosRepresentLegal?.Firma??null;
            if (signaturePad6 != null)
                signaturePad6.Image = Inspeccion.InspRutinaVigAgencia?.DatosRegente?.Firma ?? null;
