...
namespace Aig.Auditoria.Components.Inspections._17_AperturaUbicacionBotiquin
{
    public partial class CapFirmas
    {
        ...
        protected async Task SaveData()
        {
            try
            {
                var result = await inspeccionService.Save_AperCamUbicBotiquin_Frima(Inspeccion);
                if (result != null)
                {
        ...
        async Task DelayToShowSignasure()
        {
            ...
            if(signaturePad5!=null)
                signaturePad5.Image = Inspeccion.InspAperCambUbicBotiquin?.DatosRepresentLegal?.Firma??null;
            if (signaturePad6 != null)
                signaturePad6.Image = Inspeccion.InspAperCambUbicBotiquin?.DatosRegente?.Firma ?? null;
        ...
        protected async Task OnSignatureChange5(ChangeEventArgs eventArgs)
        {
            ...
            Inspeccion.InspAperCambUbicBotiquin.DatosRepresentLegal.Firma = await signaturePad5.ToDataURL(signatureType);
        }
        protected async Task RemoveSignatureImg5()
        {
            Inspeccion.InspAperCambUbicBotiquin.DatosRepresentLegal.Firma = null;
        ...
        protected async Task OnSignatureChange6(ChangeEventArgs eventArgs)
        {
            ...
            Inspeccion.InspAperCambUbicBotiquin.DatosRegente.Firma = await signaturePad6.ToDataURL(signatureType);
        }
        protected async Task RemoveSignatureImg6()
        {
            Inspeccion.InspAperCambUbicBotiquin.DatosRegente.Firma = null;
